...
def test_with_webcam(system):
    ...
    while True:
        ...
        cv2.putText(
            annotated_frame,
            f"FPS: {fps}",
            (annotated_frame.shape[1] - 100, 25),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.7,
            fps_color,
            2,
        ...
        # Display
        cv2.imshow(
            "Face Detection Test - Q:Quit R:Register S:Students", annotated_frame
        )
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            register_from_image(
                system, image_path, student_id, name, department, int(year)
            )
            ...
            if student_id and name:
                register_from_webcam(
                    system, student_id, name, department, int(year)
                )
            ...
            if students:
                for s in students:
                    print(
                        ...
                        f"({s.get('department', 'N/A')}, Year {s.get('year', 'N/A')})"
                    )
            else:
