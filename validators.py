...
def validate_student_id(student_id: str) -> Tuple[bool, Optional[str]]:
    ...
    ----------
    student_id : str
        Student identifier to validate
    ...
    Examples
    --------
    >>> validate_student_id("21CS001")
    (True, None)
    >>> validate_student_id("invalid!")
    (False, "Student ID contains invalid characters")
    """
    if not student_id:
...
def validate_department(department: str) -> Tuple[bool, Optional[str]]:
    ...
    """
    if not department:
        return True, None  # Department is optional
