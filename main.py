...
class CanteenFaceDetectionApp:
    ...
    def start_camera(self):
        """Start the camera capture (webcam, RTSP stream, YouTube stream, or video file)"""
        # Video File mode (checked first)
        if config.USE_VIDEO_FILE:
            print(f"Loading video file: {config.VIDEO_FILE_PATH}")
            ...
            if not os.path.exists(video_path):
                ...
                print(f"  3. Current VIDEO_FILE_PATH: {config.VIDEO_FILE_PATH}")
                print("\nTip: To use webcam instead, set USE_VIDEO_FILE = False in config.py")
                return False
            ...
            if not self.cap.isOpened():
                ...
                print("  2. Video format is supported (mp4, avi, mov, mkv, etc.)")
                print("  3. Codec is supported by OpenCV")
                return False
            ...
            print(f"  Loop: {'Enabled' if config.VIDEO_LOOP else 'Disabled'}")
            return True
        
        # YouTube Stream mode (checked first)
        elif config.USE_YOUTUBE:
    ...
    def run_detection(self):
        ...
        while self.is_running:
            ...
            if not ret:
                # If using video file and it ended, either loop or exit
                if config.USE_VIDEO_FILE:
                    if config.VIDEO_LOOP:
                        ...
                        print("\nVideo ended. Press 'Q' to exit or 'SPACE' for final screenshot.")
                        # Wait for user input
                        while True:
